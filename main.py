...
def get_argparser():
    ...
    parser.add_argument("--img_size", type=int, default=512, help="Size of the images")
    ...
    parser.add_argument("--model", type=str, default='unet',
                        choices=['deeplabv3_resnet50',  'deeplabv3plus_resnet50',
                                 ...
                                 'deeplabv3_mobilenet', 'deeplabv3plus_mobilenet', 'unet'], help='model name')
    ...
    parser.add_argument("--finetune", action='store_true', default=False,
                        help="Finetuning from a previous model")
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels, paths) in tqdm(enumerate(loader)):
            ...
            #plt.imshow(preds[0], cmap='jet'); plt.show()
            ...
            if opts.save_val_results:
                for i in range(len(images)):
                    ...
                    #target = torch.argmax(target.squeeze(), dim=0).detach().cpu().numpy()
                    #pred = torch.argmax(pred.squeeze(), dim=0).detach().cpu().numpy()
                    
                    Image.fromarray(image).save('results/%s_image.png' % img_name)
                    Image.fromarray(target).save('results/%s_target.png' % img_name)
                    Image.fromarray(pred).save('results/%s_pred.png' % img_name)
...
def main():
    ...
    train_loader = data.DataLoader(
        train_dst, batch_size=opts.batch_size, shuffle=True, num_workers=4, drop_last=True)
    val_loader = data.DataLoader(
        val_dst, batch_size=opts.val_batch_size, shuffle=True, num_workers=4, drop_last=True)
    ...
    if opts.model == 'unet':
        model = network.UNet(n_channels=3, n_classes=num_classes)
        optimizer = torch.optim.SGD(params=[
            {'params': model.parameters(), 'lr': opts.lr},
        ], lr=opts.lr, momentum=0.9, weight_decay=opts.weight_decay)

    else:
        model = model_map[opts.model](num_classes=num_classes, output_stride=opts.output_stride)
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        if opts.finetune:
            if opts.model == 'unet':
                new_classifier = list(model.outc.children())[:-1]
                new_classifier.append(nn.Linear(64, opts.num_classes))
    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for (images, labels, path) in train_loader:
            ...
            #lab = lab[1]
            #plt.imshow(lab, cmap='jet'); plt.show()
            #exit(0)
            cur_itrs += 1
